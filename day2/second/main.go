...
func main() {
	...
	for _, report := range sliceTwoSize {
		if checkSliceOrder(report) && destination(report) || canBeMadeSafe(report) {
			count++
		}
	}
...
func checkSliceOrder(slice []int) bool {
	if len(slice) < 2 {
		return false
	}
	isIncreasing := true
...
func destination(slice []int) bool {
	for i := 0; i < len(slice)-1; i++ {
		if absInt(slice[i]-slice[i+1]) > 3 {
			return false
		}
	}
	return true
...
func canBeMadeSafe(slice []int) bool {
	for i := 0; i < len(slice); i++ {
		newSlice := append([]int{}, slice[:i]...)
		newSlice = append(newSlice, slice[i+1:]...)
		if checkSliceOrder(newSlice) && destination(newSlice) {
			return true
		}
	}
	return false
